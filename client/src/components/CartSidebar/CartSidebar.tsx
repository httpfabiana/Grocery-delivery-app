...
const CartSidebar = () => {
   ...
   const curreny = import.meta.env.VITE_CURRENCY_SYMBOL || "$";
          ...
          <div key={item.product._id} className="flex gap-3 bg-app-cream/60 rounded-xl p-3">
           ...
           <div className="flex-1 min-w-0">
            <h4 className="text-sm font-semibold truncate">
             ...
             <p>
              {curreny} {item.product.price.toFixed(2)} / {item.product.unit}
             </p>
             ...
             <div className="flex items-center justify-between mt-2">
              ...
              <div>
                <span>
                  {curreny} {(item.product.price * item.quantity).toFixed(2)}
                </span>
       ...
       {items.length > 0 && (
        <div className="p-5 border-t border-app-border space-y-3">
          <div className="flex justify-between text-sm">
           ...
           <span className="font-medium">
             {deliveryFee === 0 ? 
              <span className="text-app-success">Fee</span> : `${curreny}${deliveryFee.toFixed(2)}`}
           </span>
          </div>
          
          {deliveryFee > 0 && 
           <p className="text-xs text-app-text-light text-center">Fee delivery on orders over {curreny} 20|
          </p>}

          <div className="flex justify-between text-base font-semibold border-t border-app-border pt-3">
           <span>Total</span>
           <span>{curreny} {grandTotal.toFixed(2)}</span>
          </div>
