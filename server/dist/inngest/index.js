...
const checkLowStock = inngest.createFunction({ id: "check-low-stock", name: "", triggers: [{ event: "inventory/stock.updated" }] }, async ({ event, step }) => {
...
const autoAssignRider = inngest.createFunction({ id: 'auto-assign-rider', name: "Auto Assign Delivery Rider",
    ...
    const result = await step.run("assign-rider", async () => {
        ...
        if (!order)
            return { skiped: true, reason: "Order not found" };
        if (order.deliveryPartnerId)
            return { skiped: true, reason: "Already assigned" };
        if (["Cancelled", "Delivered"].includes(order.status))
            return { skiped: true,
                reason: `Order is ${order.status}` };
        const busyOrders = await prisma.order.findMany({ where: {
                status: {
        ...
        if (!availableRider)
            return { skiped: true, reason: "No riders available" };
