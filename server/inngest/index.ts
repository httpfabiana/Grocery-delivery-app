...
export const inngest = new Inngest({ id: "app-delivery"});

const checkLowStock = inngest.createFunction({id: "check-low-stock", name: "", triggers: 
  [{event: "inventory/stock.updated"}]}, async({ event, step}) => {
...
const sendMonthlyOffers = inngest.createFunction({id: "send-monthly-offers", name: "Monthly Payday Offers",
 triggers: [cron("0 10 1 * *")]
  ...
  const autoAssignRider = inngest.createFunction({id: 'auto-assign-rider', name: "Auto Assign Delivery Rider",
   triggers: [{event: "order/placed"}]
    ...
    const result = await step.run("assign-rider", async() => {
     ...
     if(!order) return {skiped: true, reason: "Order not found"}
     if(order.deliveryPartnerId) return {skiped: true, reason: "Already assigned"};
     if(["Cancelled", "Delivered"].includes(order.status as string)) return {skiped: true, 
      ...
      const busyRiderId = busyOrders.map((order) => order.deliveryPartnerId)

      const availableRider = await prisma.deliveryPartner.findFirst({
       where: {
        isActive: true,
        id: {
         notIn: busyRiderId as string[]
        }
       }
      })
       if(!availableRider) return { skiped: true, reason: "No riders available"}
